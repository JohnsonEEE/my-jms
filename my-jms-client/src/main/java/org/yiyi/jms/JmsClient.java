...
public class JmsClient {
    private static volatile JmsClient instance;

    private final JmsNettyClient jmsNettyClient;

    public static JmsClient get () {
        if (instance == null) {
            synchronized (JmsNettyClient.class) {
                if (instance == null) {
                    try {
    ...
    public JmsClient () throws InterruptedException {
        jmsNettyClient = new JmsNettyClient ();
    }

    public Producer createProducer (String msgModeCode, String msgModeName) {
        jmsNettyClient.login (Role.Producer, Objects.requireNonNull (MsgMode.getByCode (msgModeCode)), msgModeName);
        return new Producer (jmsNettyClient, msgModeCode, msgModeName);
    }

    public Consumer createConsumer (String msgModeCode, String msgModeName) {
        jmsNettyClient.login (Role.Consumer, Objects.requireNonNull (MsgMode.getByCode (msgModeCode)), msgModeName);
        return new Consumer (jmsNettyClient, msgModeCode, msgModeName);
    }
}
