 ...
 */
@Component
public class MessageRequestHandler implements JmsRequestHandler {
    @Autowired
    ...
    public MsgCommunicateDTO handle (MsgCommunicateDTO msgRequest, Channel channel) {
        System.out.println ("MessageRequestHandler " + ToStringBuilder.reflectionToString (msgRequest, ToStringStyle.SHORT_PREFIX_STYLE));
        MsgCommunicateDTO dto = jmsChannelGroup.getChannelInfoById (channel.id ().asLongText ());
        msgQueueGroup.enqueueMsg (dto.getMsgMode (), dto.getMsgModeName (), msgRequest.getOriginalBytes ());
        return null;
    }
}
