 ...
 */
@Component
public class LogoutRequestHandler implements JmsRequestHandler {
    @Autowired
    ...
    public MsgCommunicateDTO handle (MsgCommunicateDTO msgRequest, Channel channel) {
        System.out.println ("LogoutRequestHandler " + ToStringBuilder.reflectionToString (msgRequest, ToStringStyle.SHORT_PREFIX_STYLE));
        jmsChannelGroup.removeChannel (channel);
        MsgCommunicateDTO dto = jmsChannelGroup.getChannelInfoById (channel.id ().asLongText ());
        if (Role.Producer.getCode ().equalsIgnoreCase (dto.getRole ())) {
            msgQueueGroup.removeMsgQueue (msgRequest.getMsgMode (), msgRequest.getMsgModeName ());
        }
        return null;
    }
}
