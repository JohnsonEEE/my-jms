 ...
 */
@Component
public class LoginRequestHandler implements JmsRequestHandler {
    @Autowired
    ...
    public MsgCommunicateDTO handle (MsgCommunicateDTO msgRequest, Channel channel) {
        System.out.println ("LoginRequestHandler " + ToStringBuilder.reflectionToString (msgRequest, ToStringStyle.SHORT_PREFIX_STYLE));
        jmsChannelGroup.addChannel (channel, msgRequest);
        if (Role.Producer.getCode ().equalsIgnoreCase (msgRequest.getRole ())) {
            msgQueueGroup.createMsgQueue (msgRequest.getMsgMode (), msgRequest.getMsgModeName ());
        }
        return null;
    }
}
