 ...
 */
@Component
public class UnregisterListenerRequestHandler implements JmsRequestHandler {
    @Autowired
    ...
    public MsgType getMsgType () {
        return MsgType.UnregisterListener;
    }

    @Override
    public MsgCommunicateDTO handle (MsgCommunicateDTO msgRequest, Channel channel) {
        System.out.println ("UnregisterListenerRequestHandler " + ToStringBuilder.reflectionToString (msgRequest, ToStringStyle.SHORT_PREFIX_STYLE));
        jmsChannelGroup.unregisterListenerForChannel (channel);
        MsgCommunicateDTO dto = jmsChannelGroup.getChannelInfoById (channel.id ().asLongText ());
        msgQueueGroup.unregisterListener (dto.getMsgMode (), dto.getMsgModeName ());
        return null;
    }
}
