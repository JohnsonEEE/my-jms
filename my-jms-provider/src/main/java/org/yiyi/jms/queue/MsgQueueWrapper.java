...
public class MsgQueueWrapper {
    ...
    private volatile String queueStatus;
    private final AtomicInteger queueListenedCount = new AtomicInteger (0);
    private final Queue <byte[]> queue = new ConcurrentLinkedQueue <> ();
    private Thread ocupiedThread;
    ...
    public AtomicInteger getQueueListenedCount () {
        return queueListenedCount;
    }
    ...
    public boolean isAvaliable () {
        boolean isAvaliable = QueueStatus.Avaliable.toString ().equalsIgnoreCase (this.getQueueStatus ());
        if (!isAvaliable && ocupiedThread == Thread.currentThread ()) {
            throw new RuntimeException ("self dead lock found");
        }
        return isAvaliable;
    }

    public void setStatusAvaliable () {
        if (ocupiedThread != null && ocupiedThread != Thread.currentThread ()) {
            throw new RuntimeException ("占用queue和释放queue的线程不是同一个！");
        }
        ocupiedThread = null;
        this.setQueueStatus (QueueStatus.Avaliable.name ());
    }

    public void setStatusBusy () {
        ocupiedThread = Thread.currentThread ();
        this.setQueueStatus (QueueStatus.Busy.name ());
    }
}
